...
-- For individual INSERT/UPDATE/DELETE statements, SQL server manages its own transaction for that change on the database table.
...
-- Triggers are never called directly by our scripts, instead they are called (or "triggered") by SQL server as it performs its internal transaction.
-- Triggers are "attached" to individual tables. Tif you drop the table, then the trigger is dropped too.

--Triggers are used for:
  - Performing complex validations that cannot be done with ordinary CHECK constrains or other constraints.
    For example, I might need to compare something in this table with some other set of information from multiple tables, or against some arregates.
  - A opprtunity to do some archiving or auditing actions
  - To prevent
...
GO
-- The trigger we want to create is because we need to do some complex validation that can't be done
...
-- We have a complex business rule 
-- How can we tell if a course's cost is being increased by more than 20%? We have to do a comparison between the "before" cost and the "after" cist. we can do this check in a trigger, if we want the opportunity to do a ROLLBACK.
-- the "before" cost is reflected in the deleted table; the "after" cost is reflected in the insert table.
...
--Notes: Foreign Keys give us out "referential integrity" in our database.
